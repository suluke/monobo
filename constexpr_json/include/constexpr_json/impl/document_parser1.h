    ...
    if (isNonAggregateDocumentType(theDocInfo)) {
      ...
      if (*aType == Type::STRING) {
        ...
        for (size_t i = 0; i < static_cast<size_t>(theDocInfo.itsNumChars);) {
          ...
          if (aCharWidth <= 0)
            return aErrorResult;
          const auto [aBytes, aBytesNum] = theDestEnc.encode(aChar);
          if (aBytesNum <= 0)
            return aErrorResult;
      ...
      for (Entity &aEntity : aResult.itsEntities) {
        ...
        case Entity::STRING: {
          ...
          while (!aStr.empty()) {
            ...
            if (aCharWidth <= 0)
              return aErrorResult;
            const auto [aBytes, aBytesUsed] = theDestEnc.encode(aChar);
            if (aBytesUsed <= 0)
              return aErrorResult;
        ...
        case Entity::OBJECT: {
          ...
          const auto aDecoded = theSrcEnc.decodeFirst(aAggregateJson);
          ...
          for (;;) {
            ...
            if (isObject) {
              ...
              const auto [aColon, aColonWidth] =
                  theSrcEnc.decodeFirst(aAggregateJson);
              if (aColonWidth <= 0)
                return aErrorResult;
              if (aColon != ':')
                return aErrorResult;
              ...
              aResult.itsStrings[aNumStrings].itsPosition = aNumChars;
              while (!aKeyStr.empty()) {
                const auto [aChar, aCharWidth] = theSrcEnc.decodeFirst(aKeyStr);
                if (aCharWidth <= 0)
                  return aErrorResult;
                const auto [aBytes, aBytesUsed] = theDestEnc.encode(aChar);
                if (aBytesUsed <= 0)
